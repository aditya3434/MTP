...
class raw_env(AECEnv):
    ...
    def __init__(self, n_vehicles=3):
        ...
        self.vehicles = [Vehicle(x_initial=450+r*GAP, y_initial=520, v_initial=20) for r in range(n_vehicles)]
    ...
    def draw(self):
        ...
        self.screen.blit(finish, (750, 350))
        ...
        for agent in self.agents:
            ...
            sprite = pygame.transform.rotate(sprite, vehicle.angle)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.vehicles = [Vehicle(x_initial=400+r*GAP, y_initial=520, v_initial=20) for r in range(len(self.vehicles))]
        self.done = False
    ...
    def step(self, action):
        ...
        pid_action = self.best_action(vehicle)

        if abs(action[0]-pid_action[1]) >= 0.4:
            action[0] = pid_action[1]

        # if vehicle.x < 650:
        #     action[0] = 0

        vehicle.take_action([0, action[0]], self.dt)
        ...
        if self._agent_selector.is_last():
            ...
            for agent in self.agents:
                ...
                if vehicle.y <= 450 and vehicle.x <= 700:
                    self.rewards[agent] -= 100
                    self.done = True
                ...
                if vehicle.angle <= 3 and vehicle.x < 650:
                    self.rewards[agent] += 0.02

                if index != len(self.agents)-1:
                    if vehicle.angle > self.vehicles[index+1].angle:
                        self.rewards[agent] -= 0.03

                if vehicle.x >= 650:
                    self.rewards[agent] += 0.005*(90-abs(90-vehicle.angle))
                ...
                self.observations[agent] = [vehicle.x/800, vehicle.y/800, vehicle.angle/100]
        else:
            self._clear_rewards()
